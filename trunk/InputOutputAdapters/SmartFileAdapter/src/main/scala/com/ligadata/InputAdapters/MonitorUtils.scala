...
import com.ligadata.InputAdapters.FileType._
...
object MonitorUtils {
  ...
  def isValidFile(fileHandler: SmartFileHandler, checkFileTypes: Boolean): Boolean = {
    try {
...
object SmartFileHandlerFactory {
  ...
  def archiveFile(adapterConfig: SmartFileAdapterConfiguration, srcFileDir: String, srcFileBaseName: String, componentsMap: scala.collection.immutable.Map[String, String]): Boolean = {
    ...
    val values = partitionFormats.map(fmt => {
      componentsMap.getOrElse(fmt, "default").toString.trim
    })
