        ...
        override def run(): Unit = {
          while (!isShutdown) {
            try {
              ...
              logger.warn("assignFilesToRequestThread: leaderCallbackRequests:%d, ReqQ:%d, ProcessingQ:%d".format(leaderCallbackRequests.size, fileRequestsQueue.size, processingFilesQueue.size));
