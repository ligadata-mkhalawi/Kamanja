...
class FileMessageExtractor(parentExecutor: ExecutorService,
  ...
  private val maxlen: Int = adapterConfig.monitoringConfig.workerBufferSize * 1024 * 1024
  //in MB
  ...
  private def extractMessages(chunk: Array[Byte], len: Int): Int = {
    ...
    if (adapterConfig.monitoringConfig.EntireFileAsOneMessage) {
      val smartFileMessage = new SmartFileMessage(chunk, 0, fileHandler, 0)
      messageFoundCallback(smartFileMessage, consumerContext)
