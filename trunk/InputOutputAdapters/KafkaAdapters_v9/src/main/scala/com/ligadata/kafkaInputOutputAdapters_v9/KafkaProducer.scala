...
object KafkaProducer extends OutputAdapterFactory {
  ...
  // Statistics Keys
  val ADAPTER_DESCRIPTION = "Kafka 0.9.0.1 Producer Client"
...
class KafkaProducer(val inputConfig: AdapterConfiguration, val nodeContext: NodeContext) extends OutputAdapter {
  ...
  //workaround for Kafka 0.9 bug with class loader
  ...
  private var retryExecutor: ExecutorService = Executors.newFixedThreadPool(1, Utils.GetScalaThreadFactory("Adapter:" + inputConfig.Name + "-ExecutorService-%d"))
