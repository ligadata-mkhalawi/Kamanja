...
//  }
  ...
  //Schedule
  def MakeSchedule(name: String, startTime: String, endTime: String, cronJobPattern: String, payload: Array[String], nameSpace: String, version: String): ScheduleDef = {
    ...
    sch.ver = version.toLong
    sch
  }

  def AddSchedule(ci: ScheduleDef,ignoreExistingObjectsOnStartup: Boolean = false): Unit = {
    val key = "%s.%s.%d".format(ci.nameSpace.trim.toLowerCase,ci.name.trim.toLowerCase,ci.Version)
