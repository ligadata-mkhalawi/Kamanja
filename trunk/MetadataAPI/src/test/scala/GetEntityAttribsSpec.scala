...
class GetEntityFieldsSpec extends FunSpec with LocalTestFixtures with BeforeAndAfter with BeforeAndAfterAll with GivenWhenThen {
  var res: String = null;
  ...
  describe("Scan all Metadata objects") {
    ...
    it("Test GetEntityFields on Metadata Objects") {
	  ...
	  logger.info("objType => %s,typeName => %s, objName => %s".format(objType,typeName,objName))
          objType match {
            case "messagedef" | "containerdef" => {
              var result = MetadataAPIImpl.GetEntityFields(objName);
