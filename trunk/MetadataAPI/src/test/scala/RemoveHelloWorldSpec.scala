...
class RemoveHelloWorldModelSpec extends FunSpec with LocalTestFixtures with BeforeAndAfter with BeforeAndAfterAll with GivenWhenThen {
  var res: String = null;
  ...
  describe("Unit Tests for all MetadataAPI operations") {
    ...
    it("Remove com.ligadata.kamanja.samples.models.HelloWorld Java Models") {
      var nameSpace = "com.ligadata.kamanja.samples.models"
      var objName = "HelloWorldModel"
      var modelName = nameSpace + "." + objName;


      var version = "0000000000000000001"
      var fullModelName = modelName + "." + version;
      And("GetModelDef API to fetch the model with the version " + fullModelName)
      res = MetadataAPIImpl.GetModelDef(nameSpace, objName, "XML", version, userid)
      res should include regex ("\"Status Code\" : -1")

      version = "0000000000000000002"
      fullModelName = modelName + "." + version;
      And("GetModelDef API to fetch the model with the version " + fullModelName)
      res = MetadataAPIImpl.GetModelDef(nameSpace, objName, "XML", version, userid)
      res should include regex ("\"Status Code\" : 0")
      ...
      fullModelName = modelName + "." + version;
      And("RemoveModel API to remove the model with version " + fullModelName)
      res = MetadataAPIImpl.RemoveModel(modelName,version, userid)
      res should include regex ("\"Status Code\" : -1")
