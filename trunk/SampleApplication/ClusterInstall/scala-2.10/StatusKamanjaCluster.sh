    ...
    if [ ! -z "$pidvals" ]; then
       if [ -n "$pidvals" ]; then
          ssh -o StrictHostKeyChecking=no -T $machine  <<-EOF
             ...
             ps u -p $pidvals | grep "kamanjamanager_2.10-1.5.1.jar" | grep -v "grep" | wc -l > "$installDir/run/$statusfile"
