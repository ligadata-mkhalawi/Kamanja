...
if [ "$1" != "debug" ]; then
	java $JAAS_CONFIG_OPT $KERBEROS_CONFIG_OPT $KEYSTORE_CONFIG_OPT $KEYSTORE_PASS_CONFIG_OPT $TRUSTSTORE_CONFIG_OPT $TRUSTSTORE_PASS_CONFIG_OPT -Dlog4j.configurationFile=file:{InstallDirectory}/config/log4j2.xml -cp {InstallDirectory}/lib/system/ExtDependencyLibs2_2.10-1.5.0.jar:{InstallDirectory}/lib/system/ExtDependencyLibs_2.10-1.5.0.jar:{InstallDirectory}/lib/system/KamanjaInternalDeps_2.10-1.5.0.jar:{InstallDirectory}/lib/system/kamanjamanager_2.10-1.5.0.jar com.ligadata.KamanjaManager.KamanjaManager --config {InstallDirectory}/config/Engine1Config_Cassandra.properties
else
