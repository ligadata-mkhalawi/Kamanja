...
class KamanjaServer(port: Int) extends Runnable {
  ...
  private var exec: ExecutorService = scala.actors.threadpool.Executors.newFixedThreadPool(5, Utils.GetScalaThreadFactory("Class:" + getClass.getName + "-exec-%d"))
...
class KamanjaManager extends Observer {
  ...
  private var metricsService: ExecutorService = scala.actors.threadpool.Executors.newFixedThreadPool(1, Utils.GetScalaThreadFactory("Class:" + getClass.getName + "-metricsService-%d"))
  ...
  def RecreateExecCtxtsCommitPartitionOffsetPool(): Unit = synchronized {
    ...
    execCtxtsCommitPartitionOffsetPool = scala.actors.threadpool.Executors.newFixedThreadPool(1, Utils.GetScalaThreadFactory("Class:" + getClass.getName + "-execCtxtsCommitPartitionOffsetPool-%d"))
...
*/
  ...
  private def run(args: Array[String]): Int = {
    ...
    val scheduledThreadPool = Executors.newScheduledThreadPool(3, Utils.GetJavaThreadFactory("Class:" + getClass.getName + "-scheduledThreadPool-%d"));
