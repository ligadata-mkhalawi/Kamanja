      ...
      try {
        element match {
          ...
          case e: PmmlModelElement => {
            ...
            result = mdMan.add(e.elementType, e.filename, Some(KamanjaEnvironmentManager.getAllTenants(0).tenantId), Some(e.modelType), None, Some(e.modelName), Some("0.0.1"), Some(e.msgConsumed), None, e.msgProduced)
          }
          case e: PythonModelElement => {
            logger.info(s"Adding PYTHON model from file '${e.filename}' with message consumed '${e.msgConsumed}'")
            result = mdMan.add(e.elementType, e.filename, Some(KamanjaEnvironmentManager.getAllTenants(0).tenantId), Some(e.modelType), None, Some(e.modelName), Some("0.0.1"), Some(e.msgConsumed), None, e.msgProduced,Some(e.modelOptions))
          }
