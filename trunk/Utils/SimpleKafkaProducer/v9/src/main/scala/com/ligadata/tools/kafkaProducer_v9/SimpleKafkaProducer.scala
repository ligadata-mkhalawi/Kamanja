...
package com.ligadata.tools.kafkaProducer_v9
...
//import kafka.client. .producer.{ ProducerConfig, Producer, KeyedMessage, Partitioner }
...
class CustPartitioner extends Partitioner {
  ...
  override def configure(congifg: java.util.Map[String,_]) = {}

  override def partition(topic: String, key: Any, keyBytes: Array[Byte], value: Any, valueBytes: Array[Byte], cluster: org.apache.kafka.common.Cluster): Int = {
    if (key == null) return 0
    try {
      if (key.isInstanceOf[Array[Byte]]) {
        var a_numPartitions = cluster.availablePartitionsForTopic(topic).size()
  ...
  * Just a local debug method
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (sAllValidTrimFls.size > 0) {
      ...
      val executor = Executors.newFixedThreadPool(flsLists, Utils.GetScalaThreadFactory("Adapter:" + inputConfig.Name + "-executor-%d"))
