...
class CsvSerDeser extends SerializeDeserialize {
  ...
  def emitField(sb: StringBuilder
    ...
    if (rawValue != null) {
      fld = valueType.getTypeCategory match {
        ...
        case ARRAY => {
          emitArray(sb, valueType, rawValue.asInstanceOf[Array[_]]); ""
        }
  ...
  override def configure(objResolver: ObjectResolver
    ...
    try {
      ...
      case e: Exception => {
        val errStr = "Unsupported csvParser type " + _csvParser + ", types kamanja support <Default,Excel,InformixUnload,InformixUnloadCsv,MySQL,RFC4180,TDF>. Given Config:" + _config.mkString(",")
        Error(errStr, e)
      }
    }
