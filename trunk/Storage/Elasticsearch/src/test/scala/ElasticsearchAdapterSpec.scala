...
package com.ligadata.automation.unittests.elasticsearchadapter
...
class ElasticsearchAdapterSpec extends FunSpec with BeforeAndAfter with BeforeAndAfterAll with GivenWhenThen {
  ...
  private var elasticsearchAdapter: ElasticsearchAdapter = null
  serializer = SerializerManager.GetSerializer("kryo")

  var dataStoreInfo = """{"StoreType": "elasticsearch","SchemaName":"bofauser","Location": "localhost","portnumber":"9300","clusterName":"elasticsearch"}"""

  private val maxConnectionAttempts = 10;
  var cnt: Long = 0
  private val containerName = "customer1"
  ...
  private def CreateAdapter: DataStore = {
    ...
    while (connectionAttempts < maxConnectionAttempts) {
      try {
        adapter = ElasticsearchAdapter.CreateStorageAdapter(kvManagerLoader, dataStoreInfo, null, null)
        return adapter
      } catch {
        case e: Exception => {
  ...
  override def beforeAll = {
    try {
      ...
      logger.info("Initialize ElasticsearchAdapter")
  ...
  describe("Unit Tests for all elasticsearchadapter operations") {

    // validate property setup
    it("Validate api operations") {
      val containerName = "customer1"

      elasticsearchAdapter = adapter.asInstanceOf[ElasticsearchAdapter]
      ...
      var srcTableName = elasticsearchAdapter.getTableName(srcContainerName)
      exists = adapter.isTableExists(srcTableName)
      assert(exists == true)

      And("Test the existence of the destination table")
      var destTableName = elasticsearchAdapter.getTableName(destContainerName)
      ...
      And("Shutdown elasticsearch session")
      noException should be thrownBy {
        adapter.Shutdown
      }
