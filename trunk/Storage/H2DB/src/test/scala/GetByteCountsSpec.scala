...
package com.ligadata.automation.unittests.h2dbadapter
...
import com.ligadata.keyvaluestore.H2dbAdapter
...
class GetByteCountsSpec extends FunSpec with BeforeAndAfter with BeforeAndAfterAll with GivenWhenThen {
  ...
  // In embedded connectionMode, I ran into the following error
  // by executing the test component in sbt console more than once
  // java.sql.SQLException: Cannot create PoolableConnectionFactory (Database may be already in use: null. Possible solutions: close all other connection(s); use the server mode [90020-192])
  // H2DB docs indicated that we need to use server and tcp connectionMode to prevent this error
  //val dataStoreInfo = """{"StoreType": "h2db","connectionMode": "tcp","SchemaName": "kamanja","Location": "localhost","portnumber": "9100","user": "test","password": "test"}"""
  private var h2dbAdapter: H2dbAdapter = null

  private def CreateAdapter: DataStore = {
    var connectionAttempts = 0
    while (connectionAttempts < maxConnectionAttempts) {
      try {
        adapter = H2dbAdapter.CreateStorageAdapter(kvManagerLoader, dataStoreInfo, null, null)
        return adapter
      } catch {
        case e: Exception => {
  ...
  describe("Unit Tests for all h2dbadapter operations") {

    // validate property setup
    it("Validate api operations") {
      val containerName = "sys.customer1"
      h2dbAdapter = adapter.asInstanceOf[H2dbAdapter]
      val tableName = h2dbAdapter.toFullTableName(containerName)
